...
void AnimManager::listAnims(const char* pSearchPatten) const
{
	...
	{
		X_LOG0("Anim", "^2%-32s^7 Refs:^2%i^7 Dur: ^2%gms^7 Frames: ^2%" PRIi32 "^7 Bones: ^2%" PRIi32 "^7 Fps: %" PRIi32,
			pAnim->getName().c_str(), pAnim->getRefCount(), pAnim->getDuration().GetMilliSeconds(), 
			pAnim->getNumFrames(), pAnim->getNumBones(), pAnim->getFps());
	}
