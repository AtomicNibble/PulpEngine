...
public:
	...
	// the resource id.
	X_INLINE const TexID getTexID() const X_OVERRIDE{ return XBaseAsset::getID(); };

	X_INLINE const char* getName() const X_OVERRIDE{ return FileName.c_str(); }
	X_INLINE const Vec2<uint16_t> getDimensions() const X_OVERRIDE{ return dimensions; }
	X_INLINE const int getWidth() const X_OVERRIDE{ return dimensions.x; }
	X_INLINE const int getHeight() const X_OVERRIDE{ return dimensions.y; }
	X_INLINE const int getNumFaces() const X_OVERRIDE{ return numFaces; }
	X_INLINE const int getDepth() const X_OVERRIDE{ return depth; }
	X_INLINE const int getNumMips() const X_OVERRIDE { return static_cast<int>(numMips); }
	X_INLINE const int getDataSize() const X_OVERRIDE { return 0; } // TODO

	X_INLINE const bool isLoaded() const X_OVERRIDE{ return !flags.IsSet(TextureFlags::LOAD_FAILED); }
	X_INLINE const bool IsShared() const X_OVERRIDE { return false; }
	X_INLINE const bool IsStreamable() const X_OVERRIDE{ return false; }

	X_INLINE const TextureType::Enum getTextureType() const X_OVERRIDE { return type; }
