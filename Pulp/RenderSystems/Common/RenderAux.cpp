	...
	AuxSortedPushBuffer& auxSortedPushBuffer) const
{
	...
	X_ASSERT(end <= auxPushBuffer.size(), "invalid range")(end);
	...
	for (AuxPushBuffer::const_iterator it(auxPushBuffer.begin() + begin), 
		itEnd(auxPushBuffer.begin() + end); it != itEnd; ++it)
	...
	const XAuxGeomRenderFlags& renderFlags)
{
	...
	if (false == auxPushBuffer.empty() &&
		auxPushBuffer[auxPushBuffer.size() - 1].renderFlags == renderFlags &&
		auxPushBuffer[auxPushBuffer.size() - 1].transMatrixIdx == GetTransMatrixIndex() &&
