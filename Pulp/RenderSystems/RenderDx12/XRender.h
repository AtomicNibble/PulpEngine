...
public:
    ...
    IPixelBuffer* createColorBuffer(core::string_view nickName, Vec2i dim, uint32_t numMips,
        texture::Texturefmt::Enum fmt, Color8u clearCol) X_FINAL;

    IRenderTarget* getCurBackBuffer(uint32_t* pIdx = nullptr) X_FINAL;

    BufferHandle createBuffer(uint32_t elementSize, uint32_t numElements, const void* pInitialData, BufUsage::Enum usage, CpuAccessFlags accessFlag) X_FINAL;
    ...
    IDeviceTexture* createTexture(core::string_view nickName, Vec2i dim, texture::Texturefmt::Enum fmt, BufUsage::Enum usage, const uint8_t* pInitialData = nullptr) X_FINAL;
    ...
    shader::IHWShader* createHWShader(shader::ShaderType::Enum type, const core::string& entry, const core::string& customDefines,
		const core::string& sourceFile, shader::PermatationFlags permFlags, render::shader::VertexFormat::Enum vertFmt) X_FINAL;
