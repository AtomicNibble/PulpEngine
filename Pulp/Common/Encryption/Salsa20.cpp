...
namespace Encryption
{
	...
	void Salsa20::setKey(const uint8_t* key)
	{
		...
		if (SSEnabled())
		{
			...
			vector_[0] = convert(reinterpret_cast<const uint8_t*>(&constant_str[0]));; // 0
			vector_[3] = convert(reinterpret_cast<const uint8_t*>(&constant_str[12]));; // 12
			vector_[13] = key32[0];
			vector_[10] = key32[1];
			vector_[7] = key32[2];
			vector_[4] = key32[3];
			vector_[1] = convert(reinterpret_cast<const uint8_t*>(&constant_str[4]));; // 4
			vector_[2] = convert(reinterpret_cast<const uint8_t*>(&constant_str[8]));; // 8
			vector_[15] = key32[4];
			vector_[12] = key32[5];
			vector_[9] = key32[6];
			vector_[6] = key32[7];
