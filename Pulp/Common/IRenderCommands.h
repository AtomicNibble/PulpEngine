...
namespace Commands
{
    ...
    {
        return sizeof(ResourceStateBase) + (sizeof(render::TextureState) * MAX_TEX_STATES) + (sizeof(render::SamplerState) * MAX_SAMPLER_STATES) + (sizeof(render::ConstantBufferHandle) * MAX_CONST_BUFFERS) + (sizeof(render::BufferState) * MAX_BUFFER_STATES);
    }

    X_INLINE constexpr int32_t ResourceStateBase::getStateSize(size_t numTex, size_t numSamp, size_t numCBs, size_t numBuf)
    {
        return static_cast<int32_t>(
            (sizeof(render::TextureState) * numTex) + (sizeof(render::SamplerState) * numSamp) + (sizeof(render::ConstantBufferHandle) * numCBs) + (sizeof(render::BufferState) * numBuf));
    }
