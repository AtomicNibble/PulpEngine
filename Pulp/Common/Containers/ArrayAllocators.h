...
public:
    ...
    {
        return static_cast<T*>(arena_->allocate(sizeof(T) * num, X_ALIGN_OF(T), 0 X_MEM_IDS("Array", "T[]") X_SOURCE_INFO_MEM_CB(X_SOURCE_INFO)));
    }
...
public:
    ...
    X_INLINE T* allocate(size_t num)
    {
        return static_cast<T*>(arena_->allocate(sizeof(T) * num, alignment, 0 X_MEM_IDS("Array", "T[]") X_SOURCE_INFO_MEM_CB(X_SOURCE_INFO)));
    }
