...
uint32_t LoggerSimpleFormatPolicyStripColors::Format(LoggerBase::Line& line, const char* indentation,
    LogType::Enum type, X_SOURCE_INFO_LOG_CA(const SourceInfo& sourceInfo) const char* channel, size_t verbosity, const char* format, va_list args)
{
    int32_t bytesWritten = LoggerSimpleFormatPolicy::Format(line, indentation, type, X_SOURCE_INFO_LOG_CA(sourceInfo) channel, verbosity, format, args);
