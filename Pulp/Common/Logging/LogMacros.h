...
#	define X_LOG0(channel, format, ...)							( gEnv == 0 ? 0 : gEnv->pLog->Log(X_SOURCE_INFO, channel, 0, format, __VA_ARGS__))
#	define X_LOG1(channel, format, ...)							( gEnv == 0 ? 0 : gEnv->pLog->Log(X_SOURCE_INFO, channel, 1, format, __VA_ARGS__))
#	define X_LOG2(channel, format, ...)							( gEnv == 0 ? 0 : gEnv->pLog->Log(X_SOURCE_INFO, channel, 2, format, __VA_ARGS__))
#	define X_WARNING(channel, format, ...)						( gEnv == 0 ? 0 : gEnv->pLog->Warning(X_SOURCE_INFO, channel, format, __VA_ARGS__))
#	define X_ERROR(channel, format, ...)						( gEnv == 0 ? 0 : gEnv->pLog->Error(X_SOURCE_INFO, channel, format, __VA_ARGS__))
#	define X_FATAL(channel, format, ...)						( gEnv == 0 ? 0 : gEnv->pLog->Fatal(X_SOURCE_INFO, channel, format, __VA_ARGS__))
...
#	define X_LOG_BULLET				core::ILog::Bullet X_PP_UNIQUE_NAME(goat)(gEnv ? gEnv->pLog : 0);
