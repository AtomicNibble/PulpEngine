...
{
	m00 = src.m00; m10 = src.m10; m20 = src.m20;
	m01 = src.m01; m11 = src.m11; m21 = src.m21;
	m02 = src.m02; m12 = src.m12; m22 = src.m22;
}
...
Matrix33<T> Matrix33<T>::createRotationV01(const Vec3<T> &v0, const Vec3<T> &v1)
{
	...
	if (dot < -0.9999)	
	{
		...
		ret.m00 = T(2 * axis.x*axis.x - 1);	ret.m01 = T(2 * axis.x*axis.y);		ret.m02 = T(2 * axis.x*axis.z);
		ret.m10 = T(2 * axis.y*axis.x);		ret.m11 = T(2 * axis.y*axis.y - 1);	ret.m12 = T(2 * axis.y*axis.z);
		ret.m20 = T(2 * axis.z*axis.x);		ret.m21 = T(2 * axis.z*axis.y);		ret.m22 = T(2 * axis.z*axis.z - 1);
	}
	else	{
		Vec3<float64_t> v = Vec3<float64_t>(v0.cross(v1));
		float64_t h = 1 / (1 + dot);

		ret.m00 = T(dot + h*v.x*v.x);		ret.m01 = T(h*v.x*v.y - v.z);		ret.m02 = T(h*v.x*v.z + v.y);
		ret.m10 = T(h*v.x*v.y + v.z);		ret.m11 = T(dot + h*v.y*v.y);		ret.m12 = T(h*v.y*v.z - v.x);
		ret.m20 = T(h*v.x*v.z - v.y);		ret.m21 = T(h*v.y*v.z + v.x);		ret.m22 = T(dot + h*v.z*v.z);
	}
