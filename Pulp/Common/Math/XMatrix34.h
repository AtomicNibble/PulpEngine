...
public:
	...
	X_INLINE bool				operator==(const Matrix34<T> &rhs) const { return equalCompare(rhs, (T)EPSILON); }
	X_INLINE bool				operator!=(const Matrix34<T> &rhs) const { return !(*this == rhs); }
	...
	void				rotate(const VecT<T> &axis, T radians) { *this *= Matrix34<T>::createRotation(axis, radians); }
	// rotate by eulerRadians - Euler angles in radians (conceptually, rotate is before 'this')
	template <template <typename> class VecT>
	void				rotate(const VecT<T> &eulerRadians) { *this *= Matrix34<T>::createRotation(eulerRadians); }
	// rotate by matrix derives rotation matrix using from, to, worldUp	(conceptually, rotate is before 'this')
	template <template <typename> class VecT>
	void				rotate(const VecT<T> &from, const VecT<T> &to, const VecT<T> &worldUp) { *this *= Matrix34<T>::createRotation(from, to, worldUp); }
	...
	Vec3<T>				getTranslate() const { return Vec3<T>(m03, m13, m23); }
	...
	static Matrix34<T>	createTranslation(const Vec4<T> &v) { return createTranslation(v.xyz()); }
	...
	static Matrix34<T>	identity() { return Matrix34(1, 0, 0,  
													0, 1, 0,
													0, 0, 1, 
													0,0,0); }
	// returns 1 filled matrix
	static Matrix34<T>  one() { return Matrix34((T)1); }
	// returns 0 filled matrix
	static Matrix34<T>  zero() { return Matrix34((T)0); }
