...
static const uint32_t MAX_SCENES = 4;        // max scenes you can create (artificial limit)
...
{
    X_INLINE ActorShape() :
        actor(INVALID_HANLDE),
        pShape(nullptr)
    {
    }
    X_INLINE ActorShape(ActorHandle a, void* s) :
        actor(a),
        pShape(s)
    {
    }
...
struct HitBuffer : public HitCallback<HitType>
{
    ...
    {
        return getNbTouches() + uint32_t(this->hasBlock);
    }
    ...
    X_INLINE uint32_t getNbTouches(void) const
    {
        return this->nbTouches;
    }
    X_INLINE const HitType* getTouches(void) const
    {
        return this->touches;
    }
...
{
protected:
    X_INLINE GeometryBase(GeometryType t) :
        type_(t)
    {
    }
...
{
public:
    X_INLINE SphereGeometry() :
        GeometryBase(GeometryType::Sphere),
        radius_(0)
    {
    }
    X_INLINE SphereGeometry(float32_t ir) :
        GeometryBase(GeometryType::Sphere),
        radius_(ir)
    {
    }
...
{
public:
    X_INLINE PlaneGeometry() :
        GeometryBase(GeometryType::Plane)
    {
    }
};
...
{
public:
    X_INLINE CapsuleGeometry() :
        GeometryBase(GeometryType::Capsule),
        radius_(0),
        halfHeight_(0)
    {
    }
    X_INLINE CapsuleGeometry(float32_t radius, float32_t halfHeight) :
        GeometryBase(GeometryType::Capsule),
        radius_(radius),
...
{
public:
    X_INLINE BoxGeometry() :
        GeometryBase(GeometryType::Sphere)
    {
    }
    X_INLINE BoxGeometry(float hx, float hy, float hz) :
        GeometryBase(GeometryType::Sphere),
        halfExtents_(hx, hy, hz)
    {
    }
    X_INLINE BoxGeometry(const Vec3f& halfExtents) :
        GeometryBase(GeometryType::Sphere),
