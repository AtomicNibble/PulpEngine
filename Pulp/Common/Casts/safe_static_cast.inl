...
namespace internal
{
	...
	struct safe_static_cast_helper<false, true>
	{
		...
		static inline TO cast(FROM from)
		{
			X_ASSERT(from <= static_cast<X_NAMESPACE(core)::traits::UnsignedType<TO>::Type>(std::numeric_limits<TO>::max()), "Number to cast exceeds numeric limits.")(from, TO(), std::numeric_limits<TO>::min(), std::numeric_limits<TO>::max());
	...
	struct safe_static_cast_helper<true, false>
	{
		...
		static inline TO cast(FROM from)
		{
			...
			X_ASSERT(static_cast<X_NAMESPACE(core)::traits::UnsignedType<FROM>::Type>(from) >= std::numeric_limits<TO>::min(), "Number to cast exceeds numeric limits.")(from, TO(), std::numeric_limits<TO>::min(), std::numeric_limits<TO>::max());
			X_ASSERT(static_cast<X_NAMESPACE(core)::traits::UnsignedType<FROM>::Type>(from) <= std::numeric_limits<TO>::max(), "Number to cast exceeds numeric limits.")(from, TO(), std::numeric_limits<TO>::min(), std::numeric_limits<TO>::max());
