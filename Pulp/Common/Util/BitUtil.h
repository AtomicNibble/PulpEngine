...
namespace bitUtil
{
    ...
    // A constant for signalling that no bit is set in a value passed to ScanBits().
    ...
    // Returns the position of the first set bit found, scanning from the MSB to the LSB (e.g. 0x1 returns 0, 0x80000000 returns 31).
    // Returns NO_BIT_SET if no bit is set in the given value.
    template<typename T>
    inline unsigned int ScanBits(T value);

    // Returns the position of the first set bit found, scanning from the LSB to the MSB (e.g. 0x1 returns 0, 0x80000000 returns 31).
    // Returns NO_BIT_SET if no bit is set in the given value.
    template<typename T>
    inline unsigned int ScanBitsForward(T value);

    // Returns whether a given integer is a power-of-two.
    // 0 is considered to be a power-of-two in this case.
    template<typename T>
