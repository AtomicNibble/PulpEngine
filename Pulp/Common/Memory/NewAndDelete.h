...
namespace Mem
{
    ...
    // "new"'s an array of POD instances, similar to calling <tt>new POD[N]</tt>.
    // Only allocates the memory needed for the instances, and does not call any constructors.
    template<typename T>
    ...
    // "new"'s an array of non-POD instances, similar to calling <tt>new NonPOD[N]</tt>.
    // Allocates memory needed by the instances, and calls the instances' constructors in order. Because
    ...
    // "delete"'s an array of instances, similar to calling <tt>delete[] instances</tt>.
    // The function delegates the actual deletion of the array to the proper function based on the type of T.
    template<typename T>
    inline void DeleteArray(T* ptr, MemoryArenaBase* arena);

    // "delete"'s an array of POD instances, similar to calling <tt>delete[] instances</tt>.
    // Only frees the memory, and does not call any destructors.
    template<typename T>
    inline void DeleteArray(T* ptr, MemoryArenaBase* arena, PODType);

    // "delete"'s an array of non-POD instances, similar to calling <tt>delete[] instances</tt>.
    // Calls the destructors in reverse order, and frees the memory.
    template<typename T>
    ...
    // Destructs an instance in memory.
    // The function delegates the actual destruction of the instance to the proper function based on the type of T.
    template<typename T>
