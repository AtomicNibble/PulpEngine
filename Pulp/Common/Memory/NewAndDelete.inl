...
namespace Mem
{
    ...
    template<typename T>
    inline T* New(MemoryArenaBase* arena, size_t alignment X_MEM_HUMAN_IDS_CB(const char* ID) X_MEM_HUMAN_IDS_CB(const char* typeName) X_SOURCE_INFO_MEM_CB(const SourceInfo& sourceInfo), NoneArenaType)
    {
        X_ASSERT_NOT_NULL(arena);
        return static_cast<T*>(arena->allocate(sizeof(T), alignment, 0 X_MEM_HUMAN_IDS_CB(ID) X_MEM_HUMAN_IDS_CB(typeName) X_SOURCE_INFO_MEM_CB(sourceInfo)));
    }

    template<typename T>
    inline T* New(MemoryArenaBase* arena, size_t alignment X_MEM_HUMAN_IDS_CB(const char* ID) X_MEM_HUMAN_IDS_CB(const char* typeName) X_SOURCE_INFO_MEM_CB(const SourceInfo& sourceInfo), ArenaType)
    {
        X_ASSERT_NOT_NULL(arena);

        MemoryArenaBase* pNewArena = static_cast<MemoryArenaBase*>(arena->allocate(sizeof(T), alignment, 0 X_MEM_HUMAN_IDS_CB(ID) X_MEM_HUMAN_IDS_CB(typeName) X_SOURCE_INFO_MEM_CB(sourceInfo)));
    ...
    template<typename T>
    inline T* NewArray(MemoryArenaBase* arena, size_t N, size_t alignment X_MEM_HUMAN_IDS_CB(const char* ID) X_MEM_HUMAN_IDS_CB(const char* typeName) X_SOURCE_INFO_MEM_CB(const SourceInfo& sourceInfo), PODType)
    {
        X_ASSERT_NOT_NULL(arena);

        // no constructors need to be called for POD types
        return static_cast<T*>(arena->allocate(sizeof(T) * N, alignment, 0 X_MEM_HUMAN_IDS_CB(ID) X_MEM_HUMAN_IDS_CB(typeName) X_SOURCE_INFO_MEM_CB(sourceInfo)));
    }

    template<typename T>
    inline T* NewArray(MemoryArenaBase* arena, size_t N, size_t alignment, size_t offset X_MEM_HUMAN_IDS_CB(const char* ID) X_MEM_HUMAN_IDS_CB(const char* typeName) X_SOURCE_INFO_MEM_CB(const SourceInfo& sourceInfo), PODType)
    {
        X_ASSERT_NOT_NULL(arena);

        // no constructors need to be called for POD types
        return static_cast<T*>(arena->allocate(sizeof(T) * N, alignment, offset X_MEM_HUMAN_IDS_CB(ID) X_MEM_HUMAN_IDS_CB(typeName) X_SOURCE_INFO_MEM_CB(sourceInfo)));
    }

    template<typename T>
    inline T* NewArray(MemoryArenaBase* arena, size_t N, size_t alignment X_MEM_HUMAN_IDS_CB(const char* ID) X_MEM_HUMAN_IDS_CB(const char* typeName) X_SOURCE_INFO_MEM_CB(const SourceInfo& sourceInfo), NonPODType)
    {
        X_ASSERT_NOT_NULL(arena);
        ...
        as_void = arena->allocate(sizeof(T) * N + sizeof(uint32_t), alignment, sizeof(uint32_t) X_MEM_HUMAN_IDS_CB(ID) X_MEM_HUMAN_IDS_CB(typeName) X_SOURCE_INFO_MEM_CB(sourceInfo));
    ...
    inline T* ConstructArray(void* where, size_t N)
    {
        ...
        return Internal::ConstructArray<T>(where, N, typename std::conjunction<std::is_scalar<T>, std::negation<std::is_volatile<T>>, std::negation<std::is_member_pointer<T>>, std::is_default_constructible<T>>::type());
    }
