    ...
    JobList::RunFlags JobList::RunJobsInternal(uint32_t threadIdx, JobListThreadState& state, bool singleJob)
    {
        ...
        do {
            ...
            for (; state.lastJobIndex < currentJob_
                   && state.lastJobIndex < safe_static_cast<int, size_t>(jobs_.size());
                 state.lastJobIndex++) {
...

            {
                if ((++fetchLock_) == 1) {
                    ...
                    for (; state.lastJobIndex <= state.nextJobIndex && state.lastJobIndex < safe_static_cast<int, size_t>(jobs_.size()); state.lastJobIndex++) {
                        if (jobs_[state.lastJobIndex].pData == &JOB_LIST_DONE) {
                            if (syncCount_ > 0) {
