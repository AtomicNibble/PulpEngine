...
// ---------------------------------------------------------------------------------------------------------------------
template<size_t N, typename TChar>
void StackString<N, TChar>::append(TChar ch)
{
    X_ASSERT(len_ + 1 < N, "Cannot append character. Not enough space left.")(len_, N);
    str_[len_++] = ch;
    str_[len_] = 0;
}
...
void StackString<N, TChar>::append(TChar ch, size_t count)
{
    ...
    X_ASSERT(len_ + count < N, "Cannot append %d character(s). Not enough space left.", count)(len_, N);
...
template<size_t N, typename TChar>
inline void StackString<N, TChar>::set(const TChar* const beginInclusive, const TChar* const endExclusive)
{
...
template<size_t N, typename TChar>
void StackString<N, TChar>::set(const TChar* const beginInclusive, size_t length)
{
    X_ASSERT(length < N, "String of length %d does not fit into StackString of size %d.", length, N)();

    memcpy(str_, beginInclusive, length);
    len_ = length;
    str_[len_] = 0;
}
...
template<size_t N, typename TChar>
inline const TChar* StackString<N, TChar>::findLast(const TChar* pBegin, const TChar* pEnd) const
{
...
template<size_t N, typename TChar>
inline const TChar* StackString<N, TChar>::find(const TChar* pBegin, const TChar* pEnd) const
{
