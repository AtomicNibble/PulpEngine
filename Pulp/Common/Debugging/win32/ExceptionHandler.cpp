...
namespace exceptionHandler
{
...

		{
			...
			if ( (Code == STATUS_ACCESS_VIOLATION || Code == STATUS_IN_PAGE_ERROR) && ExceptionRecord->NumberParameters )
			{
				...
				if ( ExpInfo )
				{
					if ( ExpInfo == 1 )
					{
						X_ERROR( "ExceptionHandler", "Information: The thread attempted to write to an inaccessible address at virtual address 0x%08X." );
					}
					else if ( ExpInfo == 8 )
					{
						X_ERROR( "ExceptionHandler", "Information: The thread caused a user-mode data execution prevention (DEP) violation at virtual address 0x%08X." );
					}
				}
				else
				{
					X_ERROR( "ExceptionHandler", "Information: The thread attempted to read the inaccessible data at virtual address 0x%08X." );
				}
			...
			X_LOG0( "ExceptionHandler", "Callstack:" );
			{
				...
				if( StackWalk64( IMAGE_FILE_MACHINE_I386, CurrentPro, CurrentThread, &stackFrame, 0, 0, SymFunctionTableAccess64, SymGetModuleBase64, 0 ) )
				{
					do
					{
					...
					while ( StackWalk64( IMAGE_FILE_MACHINE_I386, CurrentPro, CurrentThread, &stackFrame, 0, 0, SymFunctionTableAccess64, SymGetModuleBase64, 0) );
				}
			...
			if ( ContextRecord->ContextFlags & CONTEXT_CONTROL ) // AMD64 | INT
			{
				X_LOG0( "ExceptionHandler", "CPU integer registers:" );
				{
					...
					X_LOG0( "ExceptionHandler", "RBP = 0x%08X", ContextRecord->Rbp );
	...
	LONG WINAPI HandleException( _EXCEPTION_POINTERS *exceptionPointers )
	{
...

		{
			...
			if ( ContextRecord->ContextFlags & 0x10002 ) // I386 | INT
			{
				X_LOG0( "ExceptionHandler", "CPU integer registers:" );
				{
					X_LOG_BULLET;
			...
			if ( ContextRecord->ContextFlags & 0x10004 )
			{
				X_LOG0( "ExceptionHandler", "CPU segments:" );
				{
					X_LOG_BULLET;
			...
			if ( ContextRecord->ContextFlags & 0x10001 )
			{
				X_LOG0( "ExceptionHandler", "CPU control registers:" );
				{
					X_LOG_BULLET;
			...
			if ( ContextRecord->ContextFlags & 0x10010 )
			{
				X_LOG0( "ExceptionHandler", "CPU debug registers:" );
				{
					X_LOG_BULLET;
