...
private:
    struct FactoryByCName
    {
        ...
        FactoryByCName(IEngineFactory* ptr) :
            pCname(ptr ? ptr->GetName() : nullptr),
            pPtr(ptr)
        {
        }
        bool operator<(const FactoryByCName& rhs) const
        {
            return strcmp(pCname, rhs.pCname) < 0;
        }
    };
    ...
    struct FactoryByID
    {
        ...
        FactoryByID(EngineGUID id) :
            id(id),
            pPtr(nullptr)
        {
        }
        FactoryByID(IEngineFactory* ptr) :
            id(ptr ? ptr->GetGUID() : EngineGUID()),
            pPtr(ptr)
        {
        }
        bool operator<(const FactoryByID& rhs) const
        {
            if (id != rhs.id)
                return id < rhs.id;
            return pPtr < rhs.pPtr;
        }
    };
