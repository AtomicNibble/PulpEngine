...
calls the first). The first simply recieves the shader node that is being
initialized. The second recieves additional context (such as the DAG path
for the object bound to the shader) and also allows the implementation to
also send back custom user data to attach to the MRenderItem representing the
