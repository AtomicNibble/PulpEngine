...
//	r = pEngine_->RegisterObjectMethod("asset", "asset &opAssign(const asset &in)", asMETHODPR(AssetScriptProps, operator=, (const AssetScriptProps&), AssetScriptProps&), asCALL_THISCALL); BUG_CHECK(r >= 0);
	r = pEngine_->RegisterObjectMethod("asset", "assetProp@ AddVideo(const string& in, const string& in)", asMETHOD(AssetScriptProps, AddVideo), asCALL_THISCALL); BUG_CHECK(r >= 0);
	...
	r = pEngine_->RegisterObjectMethod("asset", "assetProp@ AddFx(const string& in)", asMETHOD(AssetScriptProps, AddFx), asCALL_THISCALL); BUG_CHECK(r >= 0);
