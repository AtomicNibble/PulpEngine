    ...
    const wchar_t* pInCmdLine)
{
    ...
    if (!pICore_->InitializeLoadedConverterModule(X_ENGINE_OUTPUT_PREFIX "MaterialLib", "Engine_MaterialLib")) {
        return false;
    }
    if (!pICore_->InitializeLoadedConverterModule(X_ENGINE_OUTPUT_PREFIX "ShaderLib", "Engine_ShaderLib")) {
        return false;
    }
