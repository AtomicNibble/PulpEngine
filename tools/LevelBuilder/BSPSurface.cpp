...
bspDrawSurface* BSPBuilder::DrawSurfaceForSide(const BspEntity& ent, bspBrush* b, BspSide* s, XWinding* w)
{
    ...
    for (j = 0; j < w->GetNumPoints(); j++) {
		...
		else if (si->tcGen) // added support for explicit shader texcoord generation 
		{
...
void BSPData::ClipSideByTree_r(XWinding *w, side_t *side, node_t *node)
{
	XWinding		*front, *back;
	...
	if (!node->opaque) {
		if (!side->visibleHull) {
			...
			side->visibleHull->AddToConvexHull(w, planes[side->planenum].getNormal());
		}
	}
...
void BSPData::ClipSidesByTree(uEntity_t *e)
{
	uBrush_t		*b;
	int				i;
	XWinding		*w;
...
void AddTriListToArea(uEntity_t *e, mapTri_t *triList, int planeNum, int areaNum, textureVectors_t *texVec) 
{
	uArea_t		*area;
	...
	for (group = area->groups; group; group = group->nextGroup) {
		...
		//	group->material == triList->material &&
			...
			for (i = 0; i < 2; i++) {
				for (j = 0; j < 3; j++) {
					if (math<float>::abs(texVec->v[i][j] - group->texVec.v[i][j]) > TEXTURE_VECTOR_EQUAL_EPSILON) {
						break;
					}
				}
				if (j != 3) {
				...
				if (math<float>::abs(texVec->v[i][3] - group->texVec.v[i][3]) > TEXTURE_OFFSET_EQUAL_EPSILON) {
					break;
				}
			}
			if (i == 2) {
	...
	if (!group) {
		group = (optimizeGroup_t *)malloc(sizeof(*group));
...
static void TexVecForTri(textureVectors_t *texVec, mapTri_t *tri) 
{
	...
	d0[0] = b->pos[0] - a->pos[0];
	d0[1] = b->pos[1] - a->pos[1];
	d0[2] = b->pos[2] - a->pos[2];
	d0[3] = b->uv[0] - a->uv[0];
	d0[4] = b->uv[1] - a->uv[1];

	d1[0] = c->pos[0] - a->pos[0];
	d1[1] = c->pos[1] - a->pos[1];
	d1[2] = c->pos[2] - a->pos[2];
	d1[3] = c->uv[0] - a->uv[0];
	d1[4] = c->uv[1] - a->uv[1];
	...
	temp.normalize();
	texVec->v[0] = temp;
//	texVec->v[0][3] = tri->v[0].pos * texVec->v[0] - tri->v[0].tex[0];
	...
	temp.normalize();
	texVec->v[1] = temp;
//	texVec->v[1][3] = tri->v[0].pos * texVec->v[0] - tri->v[0].tex[1];
}
...
//	}
	...
	//		tri->material = si;
			...
			for (j = 0; j < 3; j++) {
				...
				// calculate texture s/t from brush primitive texture matrix
		//		dv->st[0] = dv->pos.dot(s->texVec.v[0]) + s->texVec.v[0][3];
		//		dv->st[1] = dv->pos.dot(s->texVec.v[1]) + s->texVec.v[1][3];

				// copy normal
				dv->normal = planes[s->planenum].getNormal();
				if (dv->normal.length() < 0.9 || dv->normal.length() > 1.1) {
				//	common->Error("Bad normal in TriListForSide");
					X_ERROR("Surface", "Bad normal in TriListForSide");
				}
			}
		}
	}
		...
		//		dv->st[1] = dv->pos.dot(s->texVec.v[1]) + s->texVec.v[1][3];
				...
				//	common->Error("Bad normal in TriListForSide");
					X_ERROR("Sirface", "bad normal in normal");
				}
			}
		}
	}
...
====================
*/
void BSPData::ClipTriIntoTree_r(XWinding *w, mapTri_t *originalTri, uEntity_t *e, node_t *node)
{
	XWinding		*front, *back;
...
int BSPData::CheckWindingInAreas_r(const XWinding *w, node_t *node)
{
	XWinding		*front, *back;
...
====================
*/
void BSPData::PutWindingIntoAreas_r(uEntity_t *e, const XWinding *w, side_t *side, node_t *node)
{
	XWinding		*front, *back;
//	int				area;
...
==================
*/
void BSPData::AddMapTriToAreas(mapTri_t *tri, uEntity_t *e)
{
	int				area;
	XWinding		*w;
...
void BSPData::PutPrimitivesInAreas(uEntity_t *e)
{
	uBrush_t		*b;
	int				i;
	side_t			*side;
	...
	X_LOG0("Surface", "----- PutPrimitivesInAreas -----");

	// allocate space for surface chains for each area
	e->areas = (uArea_t *)malloc(e->numAreas * sizeof(e->areas[0]));
	...
	if (numEntities == 0) 
	{
		bool inlineAll = entities[0].mapEntity->epairs.GetBool("inlineAllStatics");
		...
		for (int eNum = 1; eNum < numEntities; eNum++) {
			...
			XRenderModel* model = renderModelManager->FindModel(modelName);

			X_LOG0("Surface", "inlining %s.\n", entity->mapEntity->epairs.GetString("name"));
			...
			Fec3f	origin = entity->mapEntity->epairs.GetVector("origin");
...
=================
*/
void BoundOptimizeGroup(optimizeGroup_t *group) 
{
	group->bounds.clear();
	for (mapTri_t *tri = group->triList; tri; tri = tri->next) {
		group->bounds.add(tri->v[0].pos);
		group->bounds.add(tri->v[1].pos);
		group->bounds.add(tri->v[2].pos);
	}
}
...
====================
*/
void BSPData::BuildLightShadows(uEntity_t *e, mapLight_t *light)
{
	int			i;
	optimizeGroup_t	*group;
	mapTri_t	*tri;
	...
	if (!light->def.parms.noShadows
		...
		for (i = 0; i < e->numAreas; i++) {
			for (group = e->areas[i].groups; group; group = group->nextGroup) {
				...
				if (planes[group->planeNum].distance(lightOrigin) > 0) {
					continue;
				}
				...
				if (!check) {
					check = (optimizeGroup_t *)malloc(sizeof(*check));
					*check = *group;
					check->triList = NULL;
...
====================
*/
void BSPData::CarveGroupsByLight(uEntity_t *e, mapLight_t *light)
{
	int			i;
	...
	for (i = 0; i < e->numAreas; i++) {
		...
		for (group = area->groups; group; group = nextGroup) {
			...
			if (!light->def.lightShader->LightEffectsBackSides() &&
				...
				planes[group->planeNum].distance(light->def.parms.origin) <= 0) {
			...
			if (inside) {
				newGroup = (optimizeGroup_t *)malloc(sizeof(*newGroup));
				*newGroup = *group;
...
=====================
*/
void BSPData::Prelight(uEntity_t *e)
{
#if 0
	int						i;
	core::TimeVal			start, end;
	mapLight_t				*light;
	...
	if (gSettings.shadowOptLevel > 0)
	{
		X_LOG0("Surface", "----- BuildLightShadows -----");
		...
		for (i = 0; i < mapLights.size(); i++) {
			light = mapLights[i];
			BuildLightShadows(e, light);
		}

		end = gEnv->pTimer->GetAsyncTime();
		X_LOG0("Surface", "%5.1f seconds for BuildLightShadows", (end - start).GetMilliSeconds());
	}

	if (!gSettings.noLightCarve)
	{
		X_LOG0("Surface", "----- CarveGroupsByLight -----");
		start = gEnv->pTimer->GetAsyncTime();
		...
		X_LOG0("Surface", "%5.1f seconds for CarveGroupsByLight", (end - start).GetMilliSeconds());
	}
#endif
